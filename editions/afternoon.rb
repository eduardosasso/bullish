...
class ClosingEdition < Edition
  def subject
    ...
    if value.start_with?(Template::MINUS)
  ...
  def indexes
    @indexes ||= {
      'sp500_c': Ticker.sp500.performance.to_s + '%',
      'nasdaq_c': Ticker.nasdaq.performance.to_s + '%',
      'dowjones_c': Ticker.dowjones.performance.to_s + '%'
    }
  end
  ...
  def top(type)
    ...
    Top.new.send(type).each_with_object({}) do |item, hash|
